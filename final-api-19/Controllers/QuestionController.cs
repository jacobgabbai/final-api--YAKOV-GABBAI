...
namespace api_final_19.Controllers
{
    ...
    public class QuestionController : ControllerBase
    {
        ...
        public QuestionController(IQuaestionRepo QuestRepo, FinalFContext context)
        {
        ...
        [HttpGet("{TestIdRef}") ]
        public async Task<ActionResult< List<Question>>> Get(string TestIdRef)
        {
            return Ok( await Task.Run(()=> _QuestRepo.T31(_context, TestIdRef)));
        }

        [HttpPost]
        public async Task<ActionResult < Question>> Post(QuestionDto3 value)
        {
            // 
            if (value.Id == "0")
            {
