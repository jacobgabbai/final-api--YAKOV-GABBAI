...
async function subTest(c, b) {
    ...
    const y112 = 'https://localhost:7115/../api/TestExControler'
        ...
        if (quest.length > 0) {
            for (let rl = 0; rl < quest.length; rl++) {
                ...
                if (response1.status == 200) {
                    ...
                    const y3 = 'https://localhost:7115/../api/AnswerEx'
...
function finishtest() {
    ...
    localStorage.setItem("stuName", stuname)
    localStorage.setItem("stuId", stuId)
    window.close("https://localhost:7115/StudentHtml/Taketest4.html")
